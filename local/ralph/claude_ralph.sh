#!/bin/bash
# Ralph Agent - Claude Code Edition
# Usage: ./claude_ralph.sh [max_iterations]
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  claude --dangerously-skip-permissions -p "$(cat "$PROMPT_FILE")" 2>&1 | tee "$TEMP_OUTPUT" || true
