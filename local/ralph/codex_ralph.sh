#!/bin/bash
# Ralph Agent - Codex CLI Edition
# Usage: ./codex_ralph.sh [max_iterations]
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  # Run Codex CLI with full-auto mode (skips all permissions)
  TEMP_OUTPUT="$SCRIPT_DIR/.ralph-output.tmp"
  codex --approval-mode full-auto -q "$(cat "$PROMPT_FILE")" 2>&1 | tee "$TEMP_OUTPUT" || true
