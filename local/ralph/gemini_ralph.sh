#!/bin/bash
# Ralph Agent - Gemini CLI Edition
# Usage: ./gemini_ralph.sh [max_iterations]
...
for i in $(seq 1 $MAX_ITERATIONS); do
  ...
  # Run Gemini CLI with yolo mode (skips all permissions)
  TEMP_OUTPUT="$SCRIPT_DIR/.ralph-output.tmp"
  gemini --yolo -p "$(cat "$PROMPT_FILE")" 2>&1 | tee "$TEMP_OUTPUT" || true
