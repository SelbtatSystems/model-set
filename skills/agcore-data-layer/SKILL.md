...
1. `CONTEXT-MAP.md`
2. `llm-wiki/db/CONTEXT.md`
3. `llm-wiki/db/SECURITY.md`
4. Relevant accepted ADRs in `llm-wiki/db/docs/adr/`, if any exist
...
**Plan mode**: use when the user asks to plan, design, scope, assess, review, or when the main agent is in Plan mode. Stay read-only. Inspect files and read-only database/storage state only. Do not create migrations, edit `init.sql`, mutate Redis/MinIO, run DDL/DML, or edit `llm-wiki/db/CONTEXT.md`.
...
When changing storage-key columns or bucket layout, keep `backend/src/storage/storage.service.ts`, owning entities, and `llm-wiki/db/CONTEXT.md` in sync. Private reads use presigned URLs; never expose internal Docker hostnames such as `minio:9000` to browsers.
