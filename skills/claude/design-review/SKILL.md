...
2. Resolve each app's `DESIGN.md` location from the project's context map (its `CLAUDE.md` / `CONTEXT-MAP.md`). In this repo, design references live at **`memory/AgCore/apps/<VaultName>/DESIGN.md`**, where `<VaultName>` is the app's vault name from CLAUDE.md's app↔vault map (e.g. `agcore-web → AgCore-web`, `myfarmjob-web → MyFarmJob`, `myfarmjob-eForm-web → eForm`).
...
|----------|--------|-------|
| 1 | The app's `DESIGN.md` (`memory/AgCore/apps/<VaultName>/DESIGN.md`) | That app's tokens, colors, components, icon/toast conventions, atmosphere — authoritative for that app's files |
