...
- Upload identity document (file input, label "Upload a copy of your identity document…"):
  `agent-browser upload @eN /home/sven/Projects/AgCore/test-docs/Test-AU-ID.png`
- Upload photo ID (file input, label "Upload your photo ID…"):
  `agent-browser upload @eN /home/sven/Projects/AgCore/test-docs/Test-Work-Permit.pdf`
...
- Visa document upload (label "Visa grant notice or visa label…"):
  `agent-browser upload @eN /home/sven/Projects/AgCore/test-docs/Test-AU-visa-letter.pdf`
- ID document upload (label "ID image (passport photo page or equivalent)"):
  `agent-browser upload @eN /home/sven/Projects/AgCore/test-docs/Test-Passport.jpg`
...
Four yes/no radios (lowercase values): previous injuries, allergies, hospital admissions, other medical. For each, if `yes`, fill the revealed "Please describe" textarea with **≥ 15 characters**. Then check the PPE acknowledgement checkbox.
...
- CEIS received checkbox ("Casual Employment Information Statement") — check (optional, default check)
- Workers' comp acknowledgement checkbox — check (required)
- Agreement accepted checkbox — check (required)
...
- Wait for the success screen — text **"Application submitted successfully!"**.
...
|---|---|---|
| Citizen (`AU_CITIZEN`) | "Upload a copy of your identity document…" | `/home/sven/Projects/AgCore/test-docs/Test-AU-ID.png` |
| Citizen (`AU_CITIZEN`) | "Upload your photo ID…" | `/home/sven/Projects/AgCore/test-docs/Test-Work-Permit.pdf` |
| Visa holder | "ID image (passport photo page or equivalent)" | `/home/sven/Projects/AgCore/test-docs/Test-Passport.jpg` |
| Visa holder | "Visa grant notice or visa label…" | `/home/sven/Projects/AgCore/test-docs/Test-AU-visa-letter.pdf` |

Use absolute paths with `agent-browser upload @eN <path>`. The backend checks magic bytes, not filenames — `.jpg`/`.png`/`.pdf` are all in the accepted MIME set.