...
After "Application submitted successfully!" appears, prove the submission landed end-to-end using the backend log tail from Step 2, then redeem the magic link and record the seeker. All of this happens in dev — emails are **logged, never sent**.
...
The dev magic link is hardcoded to `https://myfarmjob.com/auth/setup-password?token=<token>` (`backend/src/eforms/eform-submission.service.ts:1527`), which does not resolve locally. **Rewrite the host** to the myfarmjob-web dev port:

```
https://myfarmjob.com  →  http://localhost:3104
```

Keep the path and token exactly. The rewritten URL: `http://localhost:3104/auth/setup-password?token=<token>`.
...
The success page auto-redirects to `/` (~2 s). Verify:

```bash
agent-browser wait --url "**/"            # or wait 2500
agent-browser get url                     # expect http://localhost:3104/ or a dashboard route, NOT /auth/setup-password or /auth/login
agent-browser snapshot -i                 # expect authenticated seeker UI, not a login form
```
...
```json
{
  ...
  "loginUrl": "http://localhost:3104/auth/login",
...
```
  "test-link": {
    "eform-test" : "http://localhost:3103/<identifier>"
  }
```
...
After the edit, confirm the file still parses as JSON (strip `//` lines mentally, or run `node -e "JSON.parse(require('fs').readFileSync('memory/AgCore/TEST-LOGIN.md','utf8').replace(/^\s*\/\/.*/gm,''))"` — the existing file has `//` comments, so strip them before parsing).

## 5. Clean up

Kill the background log-tail PID captured in Step 2:

```bash
kill <pid> 2>/dev/null || true
```

Leave the log file in `/tmp` (useful for the report; it's outside the repo). Do not commit `memory/AgCore/TEST-LOGIN.md` — the user commits when ready.